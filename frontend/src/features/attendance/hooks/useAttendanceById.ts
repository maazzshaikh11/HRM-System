/**
 * useAttendanceById.ts
 *
 * TanStack Query hook for fetching a single attendance record by UUID.
 * Matches: GET /api/attendance/:id
 *
 * Usage:
 *   const { data, isLoading, isError, error } = useAttendanceById("some-uuid");
 */

import { useQuery, type UseQueryResult } from "@tanstack/react-query";
import {
  getAttendanceById,
  type AttendanceDetailDTO,
} from "../api/attendance.api";
import { ATTENDANCE_QUERY_KEYS } from "./queryKeys";

// ---------------------------------------------------------------------------
// Hook
// ---------------------------------------------------------------------------

/**
 * Fetches a single attendance record by its UUID.
 *
 * The query is disabled when `id` is falsy (empty string / undefined)
 * to avoid sending invalid requests during render cycles where the
 * id may not yet be available.
 *
 * @param id - The UUID of the attendance record.
 * @param enabled - Set to false to manually disable the query (default: true).
 */
export function useAttendanceById(
  id: string | undefined,
  enabled = true
): UseQueryResult<AttendanceDetailDTO, Error> {
  return useQuery<AttendanceDetailDTO, Error>({
    queryKey: ATTENDANCE_QUERY_KEYS.detail(id ?? ""),
    queryFn: () => getAttendanceById(id!),
    // Disable the query if id is not yet available
    enabled: Boolean(id) && enabled,
    staleTime: 30_000, // 30 seconds
  });
}
